...
### Slash Command & Template Deployment

When features add or modify slash command templates:
1. Source templates live in `src/specify_cli/missions/*/command-templates/`
2. A migration in `src/specify_cli/upgrade/migrations/` MUST be created to deploy
   changes to all agent directories
3. Migrations use `get_agent_dirs_for_project()` for config-aware agent processing
4. Never edit agent directory copies directly — they are generated by migrations
