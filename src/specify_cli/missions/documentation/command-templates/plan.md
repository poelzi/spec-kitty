...
Verify you are in the planning repository (not a worktree). Planning happens on the planning/base branch for all missions.

```bash
git branch --show-current  # Should show planning/base branch (meta.json -> upstream_branch, legacy fallback target_branch)
```

**Note**: Planning on the planning/base branch is standard for all spec-kitty missions. Implementation happens in per-WP worktrees.
