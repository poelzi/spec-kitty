...
2. **Generate feature directory and branch routing**:
   - **Base/planning branch**: Use the branch explicitly requested by the user for this feature. If none was provided, use the currently checked-out branch.
   - **Landing branch**: Use the generated feature slug (`###-<slug>`). This is where WP branches merge.
   - Run `spec-kitty agent feature create-feature "doc-{project-name}" --json --mission documentation --upstream-branch "<base-branch>"`
   - If the user did not provide an explicit base branch, omit `--upstream-branch` and rely on the current branch.
   - Parse JSON output for `feature`, `feature_dir`, `landing_branch`, and `upstream_branch`.
   - Feature naming convention: `doc-{project-name}` or `docs-{feature-name}` for feature-specific

3. **Update meta.json**: Update the existing `meta.json` generated by create-feature.
   - Preserve branch routing fields:
     - `target_branch` (landing branch)
     - `upstream_branch` (planning/base branch)
   - Include `mission: "documentation"` and `documentation_state`:
   ```json
   {
      "feature_number": "###",
      ...
      "source_description": "...",
      "created_at": "...",
      "target_branch": "###-doc-project-name",
      "upstream_branch": "<planning-base-branch>",
      "documentation_state": {
